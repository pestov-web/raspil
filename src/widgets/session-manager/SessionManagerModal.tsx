...
import { formatSessionDate, getSessionSummary } from '~entities/session';
...
export const SessionManagerModal: React.FC<SessionManagerModalProps> = ({
    ...
    const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (result.success) {
                    alert(`Успешно импортировано ${result.imported} сессий`);
                    loadSessions();
                } else {
                    alert(`Ошибка импорта: ${result.errors.join(', ')}`);
                }
            } catch (error) {
    ...
    const handleDelete = (sessionId: string, sessionName: string) => {
        if (confirm(`Удалить сессию "${sessionName}"?`)) {
                                ...
                                >
                                    <div className='flex items-center justify-between'>
                                        <div className='flex-1'>
                                            <h3 className='font-semibold text-lg text-gray-800 mb-1'>{session.name}</h3>
                                            {session.description && (
