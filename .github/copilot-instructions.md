...
-   **Локализация:**
    -   Все пользовательские строки проходят через `t(...)` из `react-i18next`
    -   Новые ключи добавляйте в `resources.translation` внутри `~shared/lib/i18n.ts` одновременно для RU и EN
    -   Для переключения языка используйте `changeLanguage` или хук `useLanguage` из `~features/toggle-language`
    -   Значение текущей локали хранится в `localStorage` (`LANGUAGE_STORAGE_KEY`) и синхронизируется с `<html lang>`
    -   Тесты уже импортируют i18n в `src/tests/setup.ts`; при проверке текстов обращайтесь к `i18n.t(...)`, а не к захардкоженным строкам
-   **Обратная связь:**
...
-   **Ресурсы:** все переводы лежат в `~shared/lib/i18n.ts` (разделы `translation.common`, `translation.people`, `translation.shareSessionPage` и т.д.)
-   **Подключение:** `i18n` инициализируется до рендера в `~app/App.tsx`; приложение обёрнуто в `I18nextProvider`
-   **Переключатель:** `LanguageToggle` из `~features/toggle-language` использует Headless UI `Menu` и хук `useLanguage`
-   **Синхронизация:** при смене языка обновляется `document.documentElement.lang` и атрибут `data-language`
-   **Экспорт и шаринг:** `~shared/lib/export` и `~shared/lib/share` читают строки через `t`; добавляя новые поля, не забудьте переводы для обоих языков
-   **Fallbackы:** если имя сессии пустое, используем `t('common.unnamedSession')`; не выводите необработанные пустые строки в UI
-   **Тесты:** `src/tests/share-session-page.test.tsx` получает ожидаемые тексты через `i18n.t`, это шаблон для других проверок локализованных сообщений
...
-   **Корневой компонент:** `~app/App.tsx` — RouterProvider, обёрнутый в `I18nextProvider`, для `/` и `/share`
...
-   **Локализация:** `~shared/lib/i18n.ts` (`i18n`, `SUPPORTED_LANGUAGES`, `changeLanguage`)
